...
def get_bbc(keyword, post_num):
    ...
    for x in range(post_num):
        ...
        for x in range(count):
            title = contents[x].find(attrs={'class': 'ssrcss-atl1po-PromoLink e1f5wbog0'}).text
            img_url = contents[x].find(attrs={'class': 'ssrcss-1drmwog-Image ee0ct7c0'})['src']
            url = contents[x].find(attrs={'class': 'ssrcss-atl1po-PromoLink e1f5wbog0'})['href']
